...
- **Project ref:** `livdhnxdbnhxxxlgcoge` (us-west-2)
...
- **Host:** mac-server.sudbringlab.com (Intel x86_64, macOS 15.7.4)
- **BlueBubbles:** v1.9.9, Private API disabled, standard API only (SIP remains enabled)
- **Cloudflare tunnel:** `bb.sudbringlab.cloud` → localhost:1234 (named tunnel `cerebro-bb`)
...
- **Database:** postgres
- The direct hostname (`db.livdhnxdbnhxxxlgcoge.supabase.co`) requires IPv6 and won't resolve on most networks
...
| BLUEBUBBLES_URL | imessage, digest | `https://bb.sudbringlab.cloud` |
