...
server.registerTool(
  ...
  async ({ limit, type, topic, person, days, has_file, status }) => {
    try {
      let q = supabase
        .from("thoughts")
        .select("id, content, metadata, created_at, file_url, file_type, status")
...
// Tool 8: Delete Thought by ID
server.registerTool(
  "delete_thought_by_id",
  {
    title: "Delete Thought by ID",
    description:
      "Hard-delete or soft-delete a thought by its exact UUID. Use this when you know the specific thought ID (from search or list results). Prefer this over delete_task for precise deletions.",
    inputSchema: {
      id: z.string().uuid().describe("The exact UUID of the thought to delete"),
      hard: z.boolean().optional().default(false).describe("If true, permanently removes the row. If false (default), sets status to 'deleted' (soft-delete)."),
    },
  },
  async ({ id, hard }) => {
    try {
      ...
      if (hard) {
        ...
        if (error) {
          return { content: [{ type: "text" as const, text: `Delete failed: ${error.message}` }], isError: true };
        }
        return {
          content: [{ type: "text" as const, text: `🗑️ **Permanently deleted:** ${title}\nID: ${id}` }],
        };
      } else {
        const { error } = await supabase.from("thoughts").update({ status: "deleted" }).eq("id", id);
        if (error) {
          return { content: [{ type: "text" as const, text: `Delete failed: ${error.message}` }], isError: true };
        }
        return {
          content: [{ type: "text" as const, text: `🗑️ **Soft-deleted:** ${title}\nID: ${id}` }],
        };
      }
    } catch (err: unknown) {
      return { content: [{ type: "text" as const, text: `Error: ${(err as Error).message}` }], isError: true };
...
// ============================================================
// Publishing Collection Tools (9–13)
// ============================================================
